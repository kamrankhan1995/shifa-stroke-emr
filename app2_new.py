...
def call_ai_consultant(messages: list, system_override: str = None, max_tokens: int = 900) -> str:
    ...
    try:
        ...
        if "404" in err:
            return (
                f"⚠️ **Model not found (404).** The model `{GEMINI_MODEL}` may not be available "
                f"on your API key tier. Try switching to `gemini-2.0-flash-lite` or check "
                f"[Google AI Studio](https://aistudio.google.com) for available models.\n\n`{err}`"
            )
